"""
EWMA Ablation Study - Extended Version (K=1 to K=20)
=====================================================

Tests all feature combinations from K=1 to K=20 features.
...
# Extended feature list (up to 20 features)
ALL_FEATURES = [
    # EWMA lags (most important)
    "ewma_vol_lag_1",
    "ewma_vol_lag_2", 
    "ewma_vol_lag_3",
    "ewma_vol_lag_4",
    ...
    "abs_log_return_lag_9",
    "abs_log_return_lag_10",
]
...
def prepare_data(filepath):
    """Load and prepare data with all EWMA features."""
...
def create_plots(results_df, output_dir):
    ...
    plt.suptitle("EWMA Ablation Study: Metrics vs Feature Count", fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    plot_file = os.path.join(output_dir, "ewma_ablation_metrics.png")
...
def main():
    print("=" * 60)
    print("EWMA Ablation Study - Extended (K=1 to 20)")
    print("=" * 60)
    ...
    # Prepare all data
    print("\nPreparing data with extended EWMA features...")
    all_data = {}
    ...
    for k in range(1, max_k + 1):
        ...
        if k_results:
            ...
            print(f"K={k}: MAE={avg_mae:.6f}, RMSE={avg_rmse:.6f}, R²={avg_r2:.4f}")
    
    # Save results
    results_df = pd.DataFrame(results)
    output_file = os.path.join(OUTPUT_DIR, "ewma_ablation_full.csv")
