...
"""
01_predict_zero_shot.py

Purpose:
    Runs a Zero-Shot prediction (no features) for the target date (Dec 1, 2025).
    This serves as the baseline to measure the impact of adding features.
    
    It loads historical data from ../processed_data, generates a 10-day forecast,
    and extracts the value for the target date.

Usage:
    python 01_predict_zero_shot.py
"""
...
print("Running ZERO-SHOT prediction (No Features)")
...
def predict_all():
    ...
    for filepath in tqdm(csv_files):
        ...
        # Create future rows (No features needed)
        ...
        # Predict - NO FEATURES
        context_cols = [ID_COL, DATE_COL, TARGET_COL]
        future_cols = [ID_COL, DATE_COL]
    ...
    # Output Results
    print("\n" + "="*80)
    print(f"PREDICTIONS FOR {pd.Timestamp('2025-12-01').date()} (ZERO-SHOT)")
    print("="*80)
    ...
    if not results_df.empty:
        ...
        # Save to CSV
        save_path = "predictions_2025_12_01_zeroshot.csv"
